...
nav.compass = [['NW', 'N', 'NE'], ['W', 'C', 'E'], ['SW', 'S', 'SE']];
...
nav.rotate = (dir, amount) => {
  ...
  const rotateCompassDir =
    nav.rotateArr[
      (nav.rotateArrInd[compassDir] + amount + nav.rotateArr.length) %
        nav.rotateArr.length
    ]; //BUG HERE: can't call length of rotateArrInd; used rotateArr instead
