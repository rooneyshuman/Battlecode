...
pilgrim.takeTurn = self => {
  ...
  if (!self.castle) {
    self.castle = visiblebots.filter(
      robot => robot.team === self.me.team && robot.unit === SPECS.CASTLE,
    )[0];
  }
  ...
  if (!self.destination) {
    ...
    if (
      visiblebots.filter(
        robot => robot.team === self.me.team && robot.unit === SPECS.PILGRIM,
      ).length > 1
    ) {
      // can see another pilgrim on my team
      self.resourceDestination = nav.getClosestRsrc(self.me, self.getFuelMap());
    } else {
      self.resourceDestination = nav.getClosestRsrc(
        self.me,
        self.getKarboniteMap(),
      );
    }
